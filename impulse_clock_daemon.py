...
persist_filename = "impulse_clock_time.txt"
clock_minutes = 0 # Current time displayed on clock.
    ...
    while (hours > 0):
        if (AR2_MOVEMENT == True or AR3_MOVEMENT == True):
            ...
            if (clock_minutes % MINUTES_PER_HOUR) != 0:
                ...
                advance_minutes(MINUTES_PER_HOUR - (clock_minutes % MINUTES_PER_HOUR))
            else:
...
def calculate_adjustment():
    ...
    adjust_minutes = now_minutes - clock_minutes
...
def main(argv):
    ...
    if (adjust_minutes > 0):
        ...
        if (adjust_minutes > 0):
            ...
            print (end_adjustment_now.strftime("%Y-%m-%d %H:%M:%S") + " Adjustment of", adjust_minutes, "minutes completed.")
